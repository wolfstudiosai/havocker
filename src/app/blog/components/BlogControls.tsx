...
const BlogControls: React.FC<BlogControlsProps> = ({ activeTab, setActiveTab, filter, setFilter, resetSelection }) => {
  return (
    <div className=" bg-surface/90 backdrop-blur-md border-b border-ink/5 w-full shadow-sm">
      <div className="max-w-[1920px] mx-auto px-6 md:px-12 py-3 overflow-x-auto no-scrollbar flex items-center justify-between gap-8">
          ...
          <div className="flex bg-ink/5 p-1 rounded-lg shrink-0">
          <button
            onClick={() => { setActiveTab('NEWS'); resetSelection(); }}
            className={`px-6 py-2 text-[10px] font-bold tracking-[0.2em] uppercase transition-all rounded-md ${activeTab === 'NEWS' ? 'bg-ink text-white shadow-md' : 'text-ink/40 hover:text-ink hover:bg-surface/50'}`}
          >
            LATEST NEWS
          </button>
          <button
            onClick={() => { setActiveTab('FORUM'); resetSelection(); }}
            className={`px-6 py-2 text-[10px] font-bold tracking-[0.2em] uppercase transition-all rounded-md ${activeTab === 'FORUM' ? 'bg-ink text-white shadow-md' : 'text-ink/40 hover:text-ink hover:bg-surface/50'}`}
          >
            FORUM DISC.
          </button>
        </div>
          ...
          <div className="flex items-center gap-6 min-w-max border-l border-ink/10 pl-8">
