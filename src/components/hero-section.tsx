  ...
  return (
    <section className="relative w-full md:h-screen min-h-[600px] bg-dark overflow-hidden">
      ...
      <div className="absolute bottom-24 md:bottom-32 left-4 md:left-12 z-20 pr-4">
        ...
        <h1 className="text-5xl sm:text-7xl md:text-[10rem] lg:text-[13rem] font-bold tracking-[-0.07em] text-white leading-[0.75]">
          HAVØK X1
        </h1>

        <p className="max-w-xs sm:max-w-md lg:max-w-lg text-ink/80 text-xs sm:text-sm mt-4 md:mt-8 font-bold leading-relaxed tracking-wider md:tracking-widest uppercase border-l-2 md:border-l-4 border-acid pl-4 md:pl-6 drop-shadow-md">
          PRECISION ENGINEERED FOR ADVENTURE. <br />
          <span className="text-ink">REDESIGNED FOR ELECTRIC FREEDOM.</span>
        </p>

        <div className="flex flex-wrap gap-4 md:gap-8 mt-6 md:mt-12">
          <button
            onClick={onPreOrder}
            className="flex items-center gap-2 md:gap-3 text-sm md:text-lg lg:text-xl font-bold tracking-[0.2em] md:tracking-[0.3em] text-ink hover:text-acid transition-all group uppercase"
          >
            PRE-ORDER NOW{" "}
            <ArrowRight size={24} className="group-hover:translate-x-2 transition-transform text-acid" />
          </button>
          <button className="flex items-center gap-2 md:gap-3 text-[10px] md:text-xs font-bold tracking-[0.2em] md:tracking-[0.3em] text-ink/50 hover:text-ink transition-all group uppercase">
            VIEW DETAILS
          </button>
        </div>
      </div>
      ...
      <div className="absolute bottom-24 right-4 md:right-12 z-20 hidden md:flex items-center gap-8">
        <div className="flex flex-col items-end gap-1">
          <div className="text-[9px] text-ink/40 font-mono font-bold tracking-widest">BATTERY TEMP</div>
      ...
      <div className="absolute bottom-0 w-full py-3 md:py-4 border-t border-ink/10 flex items-center overflow-hidden z-20 pointer-events-none">
        <div className="whitespace-nowrap flex animate-marquee text-[8px] md:text-[10px] font-bold tracking-[0.3em] md:tracking-[0.4em] text-ink/30 uppercase gap-10 md:gap-20">
